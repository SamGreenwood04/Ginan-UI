...
class Execution:
    def __init__(self, executable, config_path: str = GENERATED_YAML):
        """
        Initialize Execution object with YAML config and PEA executable path.

        If the YAML config file doesn't exist, it will be copied from the default template.
        """
        ...
        try:
            ...
            print(f"[Execution] ⚠️ Config file invalid or empty: {config_file} ({e})")
            print(f"[Execution] → Falling back to template: {template_file}")
            shutil.copy(template_file, config_file)
            self.config = load_yaml(config_file)
